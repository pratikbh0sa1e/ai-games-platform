...
export default function Navbar() {
  ...
  return (
    <div className="flex justify-center pt-5 px-4">
      <nav className="flex items-center gap-1 bg-black/40 backdrop-blur-xl border border-white/10 rounded-full px-3 py-2 shadow-xl">
        <Link
          to="/"
        ...
        {LINKS.map(({ to, label }) => (
          <Link
            ...
            className={`text-sm px-4 py-1.5 rounded-full transition-colors
              ${pathname === to ? "bg-white/15 text-white font-medium" : "text-gray-400 hover:text-white hover:bg-white/10"}`}
          >
            {label}
          </Link>
        ))}
